...
<div class="search">
	<form   class="meet-search-form" method="get"  >
        <!--输入框 action='meet_findnearDate.do'-->
            <!--autoFocus-->
            <div>
               <input name='findDateStart'   type="text" autocomplete="off"
                       placeholder="输入医院/医生/科室" />
            <input type="button" class="enter-btn"  value="查询"/>
<!--             <input type="submit"   value="近期查询"/> -->
        </div>
    </form>
 	<form class="meet-search-form" method="get"  >
      <select>
			...
			<div class='meet_item'>
				<div class='meet_item_p1'>
					...
					&nbsp; &nbsp;<span
						...
						&nbsp;&nbsp;<br>
						标题： <span	id="view_item" class='v_item_content'>${item_data.title  } </span>
						<br> 科室：<span	id="view_item" class='v_item_content'>${item_data.majors  } </span>
						 	<br> 内容：&nbsp;
					<span id="view_item" class='v_item_content'>${item_data.content}</span>
					<br>
					汇报数：<span id="view_item">${item_data.reportNum  } </span> 
					&nbsp;<span>,专家数：<span class='expertNum' id="view_item">${item_data.expertNum}</span></span>
					<br>
					&nbsp;群爱会诊期次：<span	 class='qaMeetNum' id="view_item">${item_data.qaMeetNum  }</span> 
...
function viewlength()
{
		$('.v_item_content').text( function (i, t){
...
// 			$(this).parent().css({   'background-color': '#f00','color':'#fff'});
	...
	$('.qaMeetNum').text ( function (i, t){
		
		if( t == '0' || t=='')
			return '-';
	});
	
	viewlength();	//2021.12.9 限制长度
});
</script>
